...
El modulo \texttt{Bezel} no solo sirve como el espacio de nombres para la biblioteca
si que además implementa varios métodos auxiliares para facilitar la configuración
y creación de clientes para acceder al servicio web C3.

\lstinputlisting[language=Ruby]{code/base.rb}
...
varios métodos para configurar el cliente. En las lineas 4-34 se definen los
metodos auxiliares mas importantes:
...
\item \texttt{client} (lineas 13-15): se inicializa una instancia \texttt{Bezel::Client} la
  cual se reutiliza en subsecuentes llamadas al método.
\item \texttt{context}:
\end{itemize}
