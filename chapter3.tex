...
uso del patron de arquitectura MVC. En Rails los modelos hacen uso de la
...
Dentro del marco de trabajo Rails tambien se tiene la biblioteca
...
de realizar la integracion de \texttt{PEAT} con el servicio web C3 haciendo uso de
esta biblioteca pero se tenian las siguientes dificultades:

\begin{itemize}
\item Presupone que el servicio web hace uso de todos los metodos HTTP (GET, POST,
...
Aunque el servicio web C3 es de tipo RESTful este no hace uso de los metodos
HTTP para realizar acciones sobre los recursos del servicio, en el servicio web
C3 todas las acciones que se quieren realizar se realizan usando el metodo HTTP
POST y se manda la accion a realizar como un parametro.

La otra dificultad es que teniamos un gran numero de asociaciones entre los
recursos defnidios por el servicio web C3 pero la biblioteca \texttt{ActiveResource}
no proporcionaba ninguna ayuda para definir estas asociaciones.

Pero la biblioteca \texttt{ActiveResource} sirvio como inspiracion para implementar
\texttt{Bezel} la cual no solamente proporcionaria acceso al servicio web C3 si no
que ademas permitiria reconstruir la estructura de tipos definida por el servidor C3
para el lenguaje de programacion Ruby, implementado la definicion de asociaciones
entre los tipos del servicio web.
...
\texttt{Bezel} permite la reconstruccion automatica de los tipos definidos por el
servidior C3 en Javascript a modelos compatibles con el marco de trabajo Rails
y el lenguaje de programacion Ruby. Esta reconstruccion automatica permite
evitar desfases entre la definicion de los tipos entre los dos sismteas y
agiliza el desarrolo del sistema \texttt{PEAt}.
...
es tipo RESTful y las capacidades de metaprogramacion de Ruby.
...
\item \textit{tenant}: indica el producto o cliente, dado que hay
  ...
  \texttt{PEAT} se tiene \textquote{peat} y \textquote{c3}.
\item \textit{module}: indica el modulo al que se esta accediendo del API
  en general se hace uso del modulo \textquote{peat}.
\item \textit{tag}: indica el tipo de ambiente, es decir si el api
  ...
  \textquote{peatprod} y \textquote{peatqa} son algunas de las tags
  que se usaron para \texttt{PEAT}.
\item \textit{type}: indica el tipo al que se quiere manipular, \textquote{Building},
...
El API del servidor no es totalmente REST principalmente porque se hace
uso del mismo verbo POST para todas las acciones, haciendo uso del
\textquote{action-name} para ejecutar la acción correspondiente. Para
...
% FIXME: dar ejemplos de ambos beneficios.
...
Asi se tiene que las clases y módulos que conforman la biblioteca son:
\begin{itemize}
...
% FIXME: Poner diagrama general de clases aqui.
