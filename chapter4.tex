...
El ambiente de produccion tiene los siguientes componentes:
\begin{itemize}
\item Equilibradores de carga: AWS tiene el servicio \textit{Elastic Load Balancing}
  (ELB) permite obtener nivel altos de tolerancia al permitir enrutar el trafico
  de forma automatica a varias instancias EC2 en varias zonas de
...
\item Oracle BD: la base de datos usada para almacenar la informacion usada
  por el servidor C3 es administrada por una base de datos Oracle Database 10g
  (Ver Figura \ref{fig:produccion} Oracle).
\item Cassandra: se tenia un cluster de nodos corriendo Cassandra donde se
  realiza analisis de los datos contenidos en la base de datos de Oracle y
  tambien se usaba como capa de cache del servidor C3.
\end{itemize}
...
redundancia al no tener un solo punto punto de error.
