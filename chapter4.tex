...
\footnote{Imágenes, hojas de estilo, codigo Javascript, etcétera} como de distribuir
la carga de peticiones para Passenger.

Se eligió integrar Nginx como servidor web por dos razones 1) la configuración es
...
análisis mucho profundo de las peticiones procesadas por el sistema \texttt{PEAT}.
...
numero se podían evitar teniendo un cache sobre modelos que se pueden considerar
estáticos. Estos modelos son:
...
\item \textit{BuildingQuestion}: este modelo representa una pregunta sobre
  ...
  total se tienen 500 preguntas. Las preguntas son actualizadas cada seis
  meses.
\item \textit{Recommendation}: este modelo representa una recomendación
  ...
  total de cien recomendaciones las cuales se actualizarían cada seis meses.
\item \textit{ValidAnswer}: las respuestas validas para una pregunta, dado
  que la mayoría de las preguntas tiene en promedio cuatro respuestas posibles
  se tiene que había dos mil respuestas posibles. Igual que las preguntas
  estas son actualizadas cada seis meses.
\end{itemize}

Se determino que la mejor solución fue que al inicializar el sistema \texttt{PEAT}
se inicializara el cache con la información de los modelos descritos anteriormente
por medio del servicio web C3. De esta forma se redujo el numero de peticiones de
forma substancial puesto pero teniendo el inconveniente de incrementarse el tiempo
...
(\textit{copy-on-write}, COW). Al iniciar el sistema usando Passenger se inicia el
proceso \texttt{Preloader} el cual en su inicialización del sistema \texttt{PEAT}
realiza la precarga de modelos en el cache. Para iniciar una nueva instancia del
sistema se realiza una copia del proceso \textit{Preloader} el cual contiene
los modelos precargados en el cache por lo que la inicialización de nuevas
instancias es casi instantánea.
