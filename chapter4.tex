...
carga, servidores de cola, etcétera.

En este contexto se encuentra \texttt{PEAT} puesto que uno de sus requerimientos
el dar soporte a por lo menos mil usuarios concurrentes, en este capitulo se
...
como de producción y las optimizaciones realizadas tanto al sistema como a la
estructura de los ambientes para permitir la mayor capacidad de carga.
...
\item Servidores EC2: son servidores con capacidades de computo que pueden ser
  ...
  del tipo \texttt{M3}\footnote{Estas instancias proporcionan un CPU virtual y
...
\item Cassandra: un cluster de nodos corriendo Cassandra donde se
  ...
  también se usa como capa de cache del servidor C3.
\end{itemize}
...
Se tienen dos equilibradores de carga el primero dirige la peticion de los usuarios
a una instancia EC2 que corre el sistema \texttt{PEAT}, el segundo equilibrador de
carga es usado para distribuir la carga de peticiones al servidor C3 por parte de
