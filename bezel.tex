...
\pagebreak
\subsubsection{Tipos principales}

Del servicio web C3 se manejaron los siguientes tipos:
\begin{itemize}
\item \textquote{User}: este tipo representa un cliente de PG\&E, solamente
  se necesita información básica como su nombre y apellido, código postal y
  su correo electrónico (Ver Figura \ref{fig:c3-types} Building).
\item \textquote{Account}: este tipo representa uno o mas contratos de servicio,
  también contiene información sobre la industria a la que pertenece el negocio
  del usuario (Ver Figura \ref{fig:c3-types} Account).
\item \textquote{Building}: este tipo representa un edificio y contiene las
  características básicas de un edificio: tipo, tamaño y antigüedad.
\item \textquote{Location}: este tipo representa una dirección de servicio,
  es decir, una ubicación física donde se prestan los servicios.
\item \textquote{BuildingProfileAnswer}: este tipo representa las respuestas
...
\item \textquote{BuildingQuestion}: este tipo representa una pregunta asociada
  a una característica de un edificio, puede tener una o varias respuestas validas
  asociadas.
\item \textquote{ValidAnswer}: este tipo representa una respuesta valida asociada
  a una pregunta sobre una característica de un edificio, cabe recordar que
  la mayoría de las preguntas tiene un numero limitado de respuestas validas.
\item \textquote{ProductCategory}: este tipo representa las categorías en las
  que se clasifican las recomendaciones, preguntas y opciones de conservación.
  Iluminación y calefacción son algunas de las categorías que se tienen
  definidas.
\item \textquote{EnergyConservationOption}: este tipo representa una medida o
  opción de conservación que se puede realizar en edificio para lograr disminuir
  su consumo energético. En este tipo se tiene la informacion generica como el
  nombre, descripcion y la informacion para realizar el calculo especifico de costo,
  ahorro y recuperación de la inversión.
\item \textquote{Recommendation}: este tipo representa la implementación de una
  opción de conservación en relación a un único edificio, es decir, contiene la
  información de costo, ahorro y recuperación de la inversión en base a las
  características del edificio asociado.
\item \textquote{RecommendationFAQ}: este tipo contiene las preguntas frecuentes
  y sus respuestas sobre la implementación y beneficios de una recomendación.
\item \textquote{ChartData}: este tipo representa toda la información de consumo
  obtenida de los medidores inteligentes instalados en las direcciones de servicio.
\end{itemize}

\rjcimagefull{1.4}{imagenes/C3-Types.png}{Diagrama tipo ER de los tipos del
...
\item \texttt{c3\_cached} (líneas 36 a 42): este método permite indicar
  ...
  tener una instancia de definida en \texttt{Bezel.cache}, si esto ocurre entonces
