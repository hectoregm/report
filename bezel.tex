...
\item \textquote{Account}: este tipo representa uno o mas contratos de servicio,
  tambien contiene informacion sobre la industria a la que pertenece el negocio
...
\item \textquote{Location}: este tipo representa una direccion de servicio,
  es decir, una ubicación fisica donde se prestan los servicios.
\item \textquote{BuildingProfileAnswer}: este tipo representa las respuestas
  dadas por el usuario o sus valores por default para las caracteristicas detalladas
  y opcionales de un edificio, tiene siempre una pregunta asociada).
\item \textquote{BuildingQuestion}: este tipo representa una pregunta asociada
  a una caracteristica detallada o opcional de un edificio, puede tener una o
  varias respuestas validas asociadas.
\item \textquote{ValidAnswer}:
\item \textquote{ProductCategory}:
\item \textquote{Recommendation}:
\item \textquote{EnergyConservationOption}
\item \textquote{RecommendationFAQ}
\item \textquote{ChartData}
\end{itemize}
