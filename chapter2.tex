...
más común de plantillas es llamado Ruby Embebido (\textit{Embedded Ruby}, ERB),
...
\item Modelo: dado que en un principio los servicios web de recomendaciones y
  ...
  \texttt{Bezel}.
  Estos cambios no tuvieron un alto impacto en el desarrollo del sistema perse,
...
Cabe mencionar que el uso de pruebas de aceptación permitió a su vez que la
...
programas (o a si mismos). Los ejemplos mas comunes de metaprogramación son los
...
dinámica para diferenciarla de la metaprogramación asociada típicamente a
...
su estado y estructura, pero además permite que un programa altere su propio estado
y estructura.
...
(\textit{Domain Specific Language} DSL) los cuales están diseñados para problemas
...
\item definir la gramática del lenguaje haciendo uso de herramientas como
  ...
  de DSL se les da el nombre de DSL externos.
\item tomar un lenguaje GPL y modificarlo para que se asemeje al DSL que
  se necesita. Este tipo de DSL se les da el nombre de DSL internos.
\end{itemize}
...
como en Java, en el cual es posible implementar DSL internos pero no dejan de
tener un gran parecido a Java por lo rígido de su sintaxis, mientras que la
sintaxis de Ruby es flexible lo que permite que los DSL internos definidos
